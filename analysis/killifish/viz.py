...
def set_syllable_cluster_ticks(syllable_cluster_labels, ax=None, axis='x', font_kw={'fontsize': 'small'}):
    """Label specified axis with syllable _cluster_ names."""

    if ax is None:
        ax = plt.gca()

    cluster_names, cluster_sizes = onp.unique(syllable_cluster_labels, return_counts=True)
...
def draw_syllable_factors(F3, syllable_cluster_labels=None, autosort=True, ax=None, im_kw={'cmap': 'magma', 'norm': mplc.LogNorm(0.5/100, 1.0)}):
    ...
    if syllable_cluster_labels is not None:
        set_syllable_cluster_ticks(syllable_cluster_labels, ax, font_kw={'fontsize': 'small', 'color':'0.4',})
