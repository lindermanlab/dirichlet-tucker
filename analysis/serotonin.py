...
def load_data():
    ...
    y = np.load(os.path.join(DATA_DIREC, "drug_ids.npy"))

    # read in behav data
    with open(os.path.join(DATA_DIREC, "behav_data.bson"), 'rb') as f:
...
@click.option('--alpha', default=1.1, help='concentration of Dirichlet prior.')
def run_sweep(seed, km_min, km_max, kn_min, kn_max, kp_min, kp_max, k_step, num_restarts, alpha):
